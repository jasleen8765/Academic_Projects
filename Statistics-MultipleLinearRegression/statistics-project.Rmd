...
ggplot(data=data_ver_2, aes(x=final_grade, group = health, fill=health)) +
  ...
  ggtitle('Histogram of Health Status vs. Final Grade')
...
ggplot(data_ver_2, aes(x=final_grade)) +
...
## 2.2- Model Building and Feature Selection
```{r}
...
X = model.matrix(Salary~.,Hitters)[, -1] #the first column (for intercept) is eliminated
y = Hitters$Salary
set.seed(50)
n = nrow(data_ver_2)
...
pred_ls = predict(fit_ls, newdata=final_grade[-idx_tr,])
...
## Non zero Predictors from LASSO are (school2+sexM+age18+age19+mom_edu3+dad_edu1+dad_edu4+mom_job2+dad_job2+reason2+guardian+study_time3+study_time4+edu_sup2+family_sup2+activities2+higher2+internet2+family_relationship3+free_time4+social_time+workday_alch2+workday_alch3+workday_alch5+weekend_alch4+health2+health3+health4+health5+absences)
